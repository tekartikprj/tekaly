...
class SyncedSdbSynchronizer extends SyncedDbSynchronizerCommon {
  ...
  Future<void> _pushLocalDirtySourceRecords(
    ...
    while (dirtySourceRecords.isNotEmpty) {
      ...
      for (var chunk in listChunk(dirtySourceRecords, stepLimitUp ?? 10)) {
        ...
        var sentList = <SyncedSdbSyncSourceRecord>[];
        ...
        var remoteWinList = <SyncedSdbSyncSourceRecord>[];
  ...
  Future<void> _syncSourceRecordDown(
    ...
    SyncedSyncStat stat, {
    SdbSyncRecord? existingSyncRecord,
  }) async {
    var syncRecord =
        (existingSyncRecord == null
              ? SdbSyncRecord()
              : db.scvSyncRecordStoreRef.record(existingSyncRecord.id).cv())
    ...
    if (existingSyncRecord == null) {
      await db.scvSyncRecordStoreRef.add(client, syncRecord);
    } else {
      // Overwrite the existing sync record, clearing the dirty flag
      // (remote wins).
      syncRecord.dirty.v = 0;
